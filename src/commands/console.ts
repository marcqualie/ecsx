...
export default class ConsoleCommand extends AwsCommand {
  ...
  async run() {
    const {
      args: { command },
      flags: { clusterKey },
    } = await this.parse(ConsoleCommand)
    ...
    if (consoleTaskConfig === undefined) {
      this.error(
        `Could not locale "${taskDefinitionName}" task in config. Please adjust config then try again`,
      )
    }

    // Find running service matching task name to get the task definition revision
    const { services: existingWebServices = [] } =
      await client.describeServices({
        cluster: clusterName,
        services: [
          'web', // NOTE: We follow convention of web/console
        ],
      })
    const activeWebService = existingWebServices.find(
      (service) => service.status === 'ACTIVE' || service.status === 'RUNNING',
    )
    const webService = activeWebService
    if (webService === undefined) {
      this.error(
        'Could not find running web service to grab container definition',
      )
    }
    ...
    const dockerTag = consoleTask?.containers?.[0]?.image
    ...
    while (
      ...
      if (taskDetails === undefined) {
        this.error(
          `Could not find task details for taskArn "${consoleTask.taskArn}"`,
        )
      }
    ...
    this.log(' ')
    this.log(
      `$ aws ecs execute-command --cluster ${clusterName} --task ${taskDetails.taskArn} --container ${taskDefinitionName} --interactive --command "${command}"`,
    )
    this.log(' ')
  }
}
