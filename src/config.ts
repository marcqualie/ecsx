...
import type {
  Configuration,
  ConfigurationClusterDefinition,
  ConfiguredVariables,
  KeyValuePairs,
  Variables,
...
export const findCluster = (
  ...
  clusterName: string,
  region: string,
): ConfigurationClusterDefinition | undefined => {
  const cluster = Object.entries(config.clusters).find(
    ([clusterKey, cluster]) => {
...
export class Config {
  ...
  parse(variables: Variables & { clusterKey?: string; taskName?: string }): {
    config: Configuration
    ...
    const combinedVariables: ConfiguredVariables = {
      ...defaultVariables,
      ...variables,
      ...(variables.clusterKey
        ? variablesFromCluster(variables.clusterKey, data)
        : {}),
    }
    ...
    for (const key of requiredVariables) {
      ...
      if (value === undefined || value === '') {
        throw new Error(
          `Missing required variable: ${key} (Please define globally, or per cluster)`,
        )
      }
    }
    ...
    for (const [envKey, envValue] of Object.entries(envVars)) {
      for (const [key, value] of Object.entries(combinedVariables)) {
        if (envValue.includes(`{{ ${key} }}`)) {
          const newValue = envValue.replace(
            new RegExp(`{{ ${key} }}`, 'g'),
            value ? value.toString() : '',
          )
